...
class Handle(Frame):
    def __init__(self, panedwindow, sash_index, disallow_dragging=False, on_click=None, **kw):
        ...
        if disallow_dragging:
            ...
            self._event_area.bind('<ButtonRelease-1>', self.root._on_release)
        
        
    def _initiate_motion(self, event) :
        self.root._active_sash = self._sash_index
...
class Vertical_Handle(Handle):
    def _on_dragging(self, event):
        y = event.y_root - self.root.winfo_rooty() - self._dy
        
        self.root.sash_place(self._sash_index, 1, y)

        self.root._update_position_all_handles()

class Horizontal_Handle(Handle):
    def _on_dragging(self, event):
        x = event.x_root - self.root.winfo_rootx() - self._dx

        self.root.sash_place(self._sash_index, x, 1)
...
if __name__ == "__main__":
    ...
    #image = PhotoImage('Kong Skull Island_Film_Poster.png')

    panedwindow = Vertical_PanedWindow(root,  sashpad = 3)
