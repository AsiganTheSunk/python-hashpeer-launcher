...
from curlstem.struct_data.curlstem_response_instance import Response
import random
from curlstem.constants.constants import DNS_SERVERS


class CurlStem:
    def __init__(self):
        self.name = self.__class__.__name__
        self.proxy_rotator = ProxyEngine()
    ...
    def _dns_setup(self):
        ...
        secure_random_dns_server, secure_random_dns_server_ip = random.SystemRandom().choice(list(DNS_SERVERS.items()))
        print(secure_random_dns_server, )
        self.handler.setopt(pycurl.DNS_SERVERS, secure_random_dns_server_ip)
    ...
    def _curl_setup(self, url, headers, attrs, ssl=True, timeout=15):
        ...
        if attrs:
            url = "%s?%s" % (url, urlencode(attrs, quote_via=quote_plus))

        self.handler.setopt(pycurl.URL, str(url))

        headers = list(map(lambda val: "%s: %s" % (val, headers[val]), headers))
    ...
    def _curl_perform(self):
        ...
        _type = self.handler.getinfo(pycurl.CONTENT_TYPE)
        data = response_buffer.getvalue()

        response_buffer.close()

        return Response(code, _type, data)
    ...
    def _dns_leak_test(self):
        # POST y 2 coockies hacen falta al menos. usar tamper geo_data
