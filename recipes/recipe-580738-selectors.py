...
class Tkinter_Selector(object):
    ...
    def match(self, root, callback, _async=True):
        queue = [(root, 0, 0)]
...
def find_all(root, selector, callback, _async=True):
    list_of_found_widgets = []
    
    if isinstance(selector, basestring):
        selector = Tkinter_Selector(selector)

    selector.match(root, callback, _async)

def find(root, selector, callback, _async=True):
    if isinstance(selector, basestring):
        selector = Tkinter_Selector(selector)
    ...
    selector.match(root, wrapper_callback, _async)

def config_extended(root, selector, _async, config_kwargs):
    if isinstance(selector, basestring):
        selector = Tkinter_Selector(selector)
...
def config_all_extended(root, selector, _async, config_kwargs):
    if isinstance(selector, basestring):
        selector = Tkinter_Selector(selector)
    
    selector.match(root, lambda widget, config_kwargs = config_kwargs: widget.configure(**config_kwargs), _async)
