...
class Text(Widget, XView, YView):
    ...
    def peer_create(self, newPathName, cnf={}, **kw): # new in Tk 8.5
        ...
        self.tk.call(self._w, 'peer', 'new', newPathName,
            *self._options(cnf, kw))
...
class Image:
    ...
    def __init__(self, imgtype, name=None, cnf={}, master=None, **kw):
        ...
        if not master:
            master = _default_root
            if not master:
                raise RuntimeError('Too early to new image')
        ...
        for k, v in cnf.items():
            if callable(v):
                v = self._register(v)
            options = options + ('-'+k, v)
        self.tk.call(('image', 'new', imgtype, name,) + options)
        self.name = name
